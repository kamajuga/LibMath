...
namespace LibMath
{
	...
	public:
		...
		public:
							...
							RowProxy(float* rowRef) { row =  rowRef; }

							RowProxy(float const* rowRef) { row = const_cast<float *>(rowRef); }
		...
		float*				data(void) { return &m_elements[0][0]; };

		static Matrix4		perspective(float const fov, float const aspectRatio, 
